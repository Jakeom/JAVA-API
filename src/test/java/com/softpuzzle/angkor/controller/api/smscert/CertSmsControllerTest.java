...
public class CertSmsControllerTest {
    ...
    @Test
    public void SMS_인증_요청() throws Exception {

        //given
        ORGReq008 request = ORGReq008.builder().phoneNumber("821011119003").uuid("0cc3d393-7ad0-4784-a2ea-31e211c37e33").build();

        //when
        ResultActions result = mvc.perform(post("/ankochat/api/smscert/req")
                .header("Language", "en")
    ...
    @Test
    public void SMS_인증_확인_가입() throws Exception {

        //given
        ORGReq009 request = ORGReq009.builder().grantType("signup").phoneNumber("821043534353").certNumber("123456").uuid("95aef0c9-d1f5-4800-b3f8-8d042a595ae6").build();

        //when
        ResultActions result = mvc.perform(post("/ankochat/api/smscert/cert")
                .header("Language", "en")
    ...
    @Test
    public void SMS_인증_확인_로그인() throws Exception {

        //given
        ORGReq005 request = ORGReq005.builder().userId("test9003").grantType("signin").phoneNumber("821011119003").certNumber("123456").uuid("0cc3d393-7ad0-4784-a2ea-31e211c37e33").build();

        //when
        ResultActions result = mvc.perform(post("/ankochat/api/smscert/certLogin/test9003")
                .header("Language", "en")
                .header("Authorization", "bearer 3601796436f2ae930b835e1fe705e58a907fc98a3fa9e4b83ab093db246f44bd")
                .content(objectMapper.writeValueAsString(request))
                .contentType(MediaType.APPLICATION_JSON)
                .accept(MediaType.APPLICATION_JSON)
        );
                ...
                .andDo(document("{class-name}/{method-name}", preprocessResponse(prettyPrint()),
                                responseFields(
                                        List.of(
                                                ...
                                                fieldWithPath("data.authKey").type(JsonFieldType.STRING).description("사용자 authKey"),
                                                fieldWithPath("data.phonecode").type(JsonFieldType.STRING).description("사용자 나라별 국가번호 코드"),
